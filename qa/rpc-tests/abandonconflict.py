...
class AbandonConflictTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txA = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), Decimal("1"))
        txB = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), Decimal("1"))
        txC = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), Decimal("1"))
        ...
        # Identify the 1btc outputs
        nA = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(txA, 1)["vout"]) if vout["value"] == Decimal("1"))
        nB = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(txB, 1)["vout"]) if vout["value"] == Decimal("1"))
        nC = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(txC, 1)["vout"]) if vout["value"] == Decimal("1"))
        ...
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance - Decimal("3.0") + Decimal("2.49996"))
        balance = newbalance
        ...
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance - Decimal("2.49996"))
        ...
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance + Decimal("3.0"))
        balance = newbalance
        ...
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance - Decimal("2.0") + Decimal("1.499998"))
        balance = newbalance

        # Send child tx again so its unabandoned
        self.nodes[0].enqueuerawtransaction(signed2["hex"],"flush")
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance - Decimal("1.0") - Decimal("1.499998") + Decimal("2.49996"))
        balance = newbalance
