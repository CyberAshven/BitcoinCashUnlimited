...
class ExcessiveBlockTest (BitcoinTestFramework):
    ...
    def testExcessiveBlockSize(self):
        ...
        self.nodes[0].setexcessiveblock(5000)
        ...
        logging.info("Test excessively sized block")
        ...
        # the blocks should never propagate
        logging.info("node1")
        self.nodes[0].generate(1)
        assert_equal(True, self.expectHeights([base + 2, base, base, base]))

        logging.info("node2")
        self.nodes[0].generate(1)
        assert_equal(True, self.expectHeights([base + 3, base, base, base]))

        logging.info("node3")
        self.nodes[0].generate(1)
        assert_equal(True, self.expectHeights([base + 4, base, base, base]))
