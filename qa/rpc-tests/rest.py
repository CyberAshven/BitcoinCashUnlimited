...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txid = self.nodes[0].sendtoaddress(self.nodes[1].getnewaddress(), COINBASE_REWARD - Decimal(".1"))
        self.sync_all()
        self.nodes[2].generate(1)
        self.sync_all()
        bb_hash = self.nodes[0].getbestblockhash()

        assert_equal(self.nodes[1].getbalance(), COINBASE_REWARD - Decimal(".1")) #balance now should be 0.1 on node 1
        ...
        assert_equal(json_obj['utxos'][0]['value'], 9.9)
        ...
        txid = self.nodes[0].sendtoaddress(self.nodes[1].getnewaddress(), Decimal(".01"))
        json_string = http_get_call(url.hostname, url.port, '/rest/tx/'+txid+self.FORMAT_SEPARATOR+"json")
        json_obj = json.loads(json_string)
        vintx = json_obj['vin'][0]['txid'] # get the vin to later check for utxo (should be spent by then)
        # get n of 0.1 outpoint
