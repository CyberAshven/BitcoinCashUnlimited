...
{
    LOCK(wallet->cs_wallet);
    for (const PAIRTYPE(const CTxDestination, CAddressBookData) & item : wallet->mapAddressBook)
        for (const PAIRTYPE(const std::string, std::string) & item2 : item.second.destdata)
