...
size_t ProjectedSBGrapheneSizeBytes(uint64_t version, uint64_t nBlockTxs, uint64_t nExcessTxs, uint64_t nSymDiff, bool computeOptimized=false)
{
...
uint256 SBGetHash(unsigned int nIndex)
{
    std::stringstream ss;
...
BOOST_FIXTURE_TEST_SUITE(graphene_sb_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(graphene_set_decodes_multiple_sizes)
{
    ...
    for (size_t nItems : nItemList)
    {
        ...
        {
            nNumHashes++;
            const uint256 &hash = SBGetHash(nNumHashes);
        ...
        // Add 10 more items to receiver mempool
        {
            ...
            {
                nNumHashes++;
                receiverItems.push_back(SerializeHash(SBGetHash(nNumHashes)));
            }
...
BOOST_AUTO_TEST_CASE(graphene_set_finds_brute_force_opt_for_small_blocks)
{
    ...
    for (a = 1; a < m - mu; a++)
    {
        size_t totalBytes = ProjectedSBGrapheneSizeBytes(version, n, m - mu, a);
        size_t totalBytesOpt = ProjectedSBGrapheneSizeBytes(version, n, m - mu, a, true);
...
BOOST_AUTO_TEST_CASE(graphene_set_approx_opt_close_to_optimal)
{
    for (uint64_t version=0; version < MAX_GRAPHENE_SET_VERSION; version++)
    {
        ...
        float totalBytesApprox = (float)ProjectedSBGrapheneSizeBytes(version, n, m - mu, CGrapheneSet::ApproxOptimalSymDiff(version, n));
        float totalBytesBrute =
            (float)ProjectedSBGrapheneSizeBytes(version, n, m - mu, grapheneSet.BruteForceSymDiff(n, m, m - mu, MAX_CHECKSUM_BITS));
        float totalBytesBruteOpt =
            (float)ProjectedSBGrapheneSizeBytes(version, n, m - mu, grapheneSet.BruteForceSymDiff(n, m, m - mu, MAX_CHECKSUM_BITS), true);
...
BOOST_AUTO_TEST_CASE(graphene_block_can_serde)
{
    ...
    // regular graphene block
    {
        ...
        CSBGrapheneBlock senderGrapheneBlock(std::make_shared<CSubBlock>(block), 5, 6, version, false);
        CSBGrapheneBlock receiverGrapheneBlock(4);
        CDataStream ss(SER_DISK, 0);
    ...
    // compute optimized graphene block
    {
        ...
        const CTransactionRef ptx = MakeTransactionRef(tx);
        block.vtx.push_back(ptx);
        CSBGrapheneBlock senderGrapheneBlock(std::make_shared<CSubBlock>(block), 5, 6, version, true);
        CSBGrapheneBlock receiverGrapheneBlock(4, true);
        CDataStream ss(SER_DISK, 0);
