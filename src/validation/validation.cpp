...
/** Delete all entries in setBobtailBlockIndexCandidates that are worse than the current tip. */
void PruneBobtailBlockIndexCandidates()
{
    AssertLockHeld(cs_main);
    if (setBobtailBlockIndexCandidates.empty())
        return; // nothing to prune

    // Note that we can't delete the current block itself, as we may need to return to it later in case a
    // reorganization to a better block fails.
    std::set<CBlockIndex *, CBlockIndexWorkComparator>::iterator it = setBobtailBlockIndexCandidates.begin();
    while (it != setBobtailBlockIndexCandidates.end() && setBobtailBlockIndexCandidates.value_comp()(*it, chainActive.Tip()))
    {
        setBobtailBlockIndexCandidates.erase(it++);
    }
}
...
bool LoadBobtailBlockIndexDB()
{
    ...
    for (const std::pair<int, CBlockIndex *> &item : vSortedByHeight)
    {
        ...
        if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS) && (pindex->nChainTx || pindex->pprev == nullptr))
            setBobtailBlockIndexCandidates.insert(pindex);
        CBlockIndex *pBestInvalid = pindexBestInvalid.load();
        if (pindex->nStatus & BLOCK_FAILED_MASK && (!pBestInvalid || pindex->nChainWork > pBestInvalid->nChainWork))
            pindexBestInvalid = pindex;
        if (pindex->pprev)
...
void UnloadBobtailBlockIndex()
{
...

    {
        ...
        nLastBlockFile = 0;
        mapUnConnectedHeaders.clear();
        setBobtailBlockIndexCandidates.clear();
        chainActive.SetTip(nullptr);
        pindexBestInvalid = nullptr;
        pindexBestHeader = nullptr;
        mapBlocksUnlinked.clear();
    ...
    while (pindex != nullptr)
    {
        ...
        if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) && pindexFirstNeverProcessed == nullptr) {
            if (pindexFirstInvalid == nullptr) {
                // If this block sorts at least as good as the current tip and
                // is valid and we have all data for its parents, it must be in
                // setBobtailBlockIndexCandidates.  chainActive.Tip() must also be there
                // even if some data has been pruned.
                ...
                //if ((!chainContainsExcessive(pindex)) && (pindexFirstMissing == nullptr || pindex ==
        chainActive.Tip()) )
                //    assert(setBobtailBlockIndexCandidates.count(pindex));

                    // If some parent is missing, then it could be that this block was in
                    // setBobtailBlockIndexCandidates but had to be removed because of the missing data.
                    // In this case it must be in mapBlocksUnlinked -- see test below.
            }
        // If this block sorts worse than the current tip or some ancestor's block has never been seen, it cannot be in
        setBobtailBlockIndexCandidates.
        } else {
            assert(setBobtailBlockIndexCandidates.count(pindex) == 0);
        }
        */
            ...
            pindexFirstMissing != nullptr)
        {
            ...
            //    tip.
            // So if this block is itself better than chainActive.Tip() and it wasn't in
            // setBobtailBlockIndexCandidates, then it must be in mapBlocksUnlinked.
            if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) && setBobtailBlockIndexCandidates.count(pindex) == 0)
            {
                if (pindexFirstInvalid == nullptr)
                {
                    assert(foundInUnlinked);
...
bool ReconsiderBobtailBlock(CValidationState &state, CBlockIndex *pindex)
{
    ...
    while (it != mapBlockIndex.end())
    {
        if (!it->second->IsValid() && it->second->GetAncestor(nHeight) == pindex)
        {
            ...
            setDirtyBlockIndex.insert(it->second);
            if (it->second->IsValid(BLOCK_VALID_TRANSACTIONS) && it->second->nChainTx &&
                setBobtailBlockIndexCandidates.value_comp()(chainActive.Tip(), it->second))
            {
                setBobtailBlockIndexCandidates.insert(it->second);
            }
            if (it->second == pindexBestInvalid.load())
            {
                // Reset invalid block marker if it was pointing to one of those.
...
int32_t UnlimitedComputeBobtailBlockVersion(const CBlockIndex *pindexPrev, const Consensus::Params &params, uint32_t nBobtailTime)
{
    if (blockVersion != 0) // BU: allow override of block version
    {
        return blockVersion;
...
CBlockIndex *FindMostBobtailWorkChain()
{
    ...
    do
    {
        ...
        // Find the best candidate header.
        {
            std::set<CBlockIndex *, CBlockIndexWorkComparator>::reverse_iterator it = setBobtailBlockIndexCandidates.rbegin();
            if (it == setBobtailBlockIndexCandidates.rend())
                return nullptr;
            pindexNew = *it;
        }
        ...
        while (pindexTest && !chainActive.Contains(pindexTest))
        {
            ...
            // Pruned nodes may have entries in setBobtailBlockIndexCandidates for
            // which block files have been deleted.  Remove those as candidates
            // for the most work chain if we come across them; we can't switch
            // to a chain unless we have all the non-active-chain parent blocks.
            fFailedChain = pindexTest->nStatus & BLOCK_FAILED_MASK;
        ...
        if (fFailedChain || fMissingData || (fRecentExcessive && !fOldExcessive))
        {
            ...
            while (pindexTest != pindexFailed)
            {
                ...
                else if (fMissingData || (fRecentExcessive && !fOldExcessive))
                {
                    // If we're missing data, then add back to mapBlocksUnlinked,
                    // so that if the block arrives in the future we can try adding
                    // to setBobtailBlockIndexCandidates again.
                    mapBlocksUnlinked.insert(std::make_pair(pindexFailed->pprev, pindexFailed));
                }
                setBobtailBlockIndexCandidates.erase(pindexFailed);
                pindexFailed = pindexFailed->pprev;
            }
            setBobtailBlockIndexCandidates.erase(pindexTest);
            fInvalidAncestor = true;
        }
...
bool InvalidateBobtailBlock(CValidationState &state, const Consensus::Params &consensusParams, CBlockIndex *pindex)
{
    ...
    // Mark the block itself as invalid.
    {
        ...
        setDirtyBlockIndex.insert(pindex);
        setBobtailBlockIndexCandidates.erase(pindex);
        ...
        while (chainActive.Contains(pindex))
        {
            ...
            setDirtyBlockIndex.insert(pindexWalk);
            setBobtailBlockIndexCandidates.erase(pindexWalk);
            // ActivateBestChain considers blocks already in chainActive
            // unconditionally valid already, so force disconnect away from it.
            if (!DisconnectTip(state, consensusParams))
            {
    ...
    // The resulting new best tip may not be in setBobtailBlockIndexCandidates anymore, so
    // add it again.
    {
        READLOCK(cs_mapBlockIndex);
        BlockMap::iterator it = mapBlockIndex.begin();
        ...
        {
            if (it->second->IsValid(BLOCK_VALID_TRANSACTIONS) && it->second->nChainTx &&
                !setBobtailBlockIndexCandidates.value_comp()(it->second, chainActive.Tip()))
            {
                setBobtailBlockIndexCandidates.insert(it->second);
            }
            it++;
    ...
    // Since this block has been invalidated and removed from setBobtailBlockIndexCandidates
    // then reset the best header to the current most work chain.
    CBlockIndex *mostWork = FindMostWorkChain();
    if (mostWork)
    {
...
void CheckBobtailForkWarningConditions()
{
    AssertLockHeld(cs_main);
    // Before we get past initial download, we cannot reliably alert about forks
    // (we assume we don't get stuck on a fork before the last checkpoint)
    ...
    // If our best fork is no longer within 72 blocks (+/- 12 hours if no one mines it)
    // of our head, drop it
    if (pindexBobtailBestForkTip && chainActive.Height() - pindexBobtailBestForkTip->nHeight >= 72)
        pindexBobtailBestForkTip = nullptr;

    if (pindexBobtailBestForkTip)
    {
        if (!fLargeWorkForkFound && pindexBobtailBestForkBase)
        {
            std::string warning = std::string("'Warning: Large-work fork detected, forking after block ") +
                                  pindexBobtailBestForkBase->phashBlock->ToString() + std::string("'");
            AlertNotify(warning);
        }
        if (pindexBobtailBestForkTip && pindexBobtailBestForkBase)
        {
            LOGA("%s: Warning: Large valid fork found\n  forking the chain at height %d (%s)\n  lasting to height "
                 "%d (%s).\nChain state database corruption likely.\n",
                __func__, pindexBobtailBestForkBase->nHeight, pindexBobtailBestForkBase->phashBlock->ToString(),
                pindexBobtailBestForkTip->nHeight, pindexBobtailBestForkTip->phashBlock->ToString());
            fLargeWorkForkFound = true;
        }
    }
    else
...
bool ReceivedBobtailBlockTransactions(const CBlock &block,
    ...
    const CDiskBlockPos &pos)
{
    AssertLockHeld(cs_main); // for setBobtailBlockIndexCandidates
    WRITELOCK(cs_mapBlockIndex); // for nStatus and nSequenceId
    ...
    if (pindexNew->pprev == nullptr || pindexNew->pprev->nChainTx)
    {
        ...
        while (!queue.empty())
        {
            ...
            pindex->nChainTx = (pindex->pprev ? pindex->pprev->nChainTx : 0) + pindex->nTx;
            pindex->nSequenceId = ++nBlockSequenceId;
            if (chainActive.Tip() == nullptr || !setBobtailBlockIndexCandidates.value_comp()(pindex, chainActive.Tip()))
            {
                setBobtailBlockIndexCandidates.insert(pindex);
            }
            std::pair<std::multimap<CBlockIndex *, CBlockIndex *>::iterator,
...
/** Store block on disk. If dbp is non-nullptr, the file is known to already reside on disk */
bool AcceptBobtailBlock(const CBlock &block,
    CValidationState &state,
    const CChainParams &chainparams,
    CBlockIndex **ppindex,
    bool fRequested,
 ...
 * @return A DisconnectResult as an int
 */
int BobtailApplyTxInUndo(Coin &&undo, CCoinsViewCache &view, const COutPoint &out)
{
    bool fClean = true;
...
bool ConnectBobtailBlockPrevalidations(const CBlock &block,
    CValidationState &state,
    CBlockIndex *pindex,
    CCoinsViewCache &view,
    const CChainParams &chainparams,
 ...
 *  before exiting ConnectBlock.
 */
static void ConnectBobtailBlockScopeExit(bool fParallel,
    CCheckQueueControl<CScriptCheck> &control,
    CCheckQueue<CScriptCheck> *pScriptQueue)
{
    // Typically the script validations would be stopped by issuing a PV->Quit() however under
...
bool ConnectBobtailBlockDependencyOrdering(const CBlock &block,
    CValidationState &state,
    CBlockIndex *pindex,
    CCoinsViewCache &view,
    const CChainParams &chainparams,
    ...
    std::vector<std::pair<uint256, CDiskTxPos> > &vPos)
{
    ...
    int64_t nBobtailTime3 = GetStopwatchMicros();
    nBobtailTimeConnect += nBobtailTime3 - nBobtailTime2;
    LOG(BENCH, "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n",
        (unsigned)block.vtx.size(), 0.001 * (nBobtailTime3 - nBobtailTime2),
        0.001 * (nBobtailTime3 - nBobtailTime2) / block.vtx.size(),
        nInputs <= 1 ? 0 : 0.001 * (nBobtailTime3 - nBobtailTime2) / (nInputs - 1), nBobtailTimeConnect * 0.000001);

    int64_t nBobtailTime4 = GetStopwatchMicros();
    nBobtailTimeVerify += nBobtailTime4 - nBobtailTime2;
    LOG(BENCH, "    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs]\n", nInputs - 1, 0.001 * (nBobtailTime4 - nBobtailTime2),
        nInputs <= 1 ? 0 : 0.001 * (nBobtailTime4 - nBobtailTime2) / (nInputs - 1), nBobtailTimeVerify * 0.000001);

    return true;
}


bool ConnectBobtailBlockCanonicalOrdering(const CBlock &block,
    CValidationState &state,
    CBlockIndex *pindex,
    CCoinsViewCache &view,
    const CChainParams &chainparams,
...
void InvalidBobtailBlockFound(CBlockIndex *pindex, const CValidationState &state)
{
    AssertLockHeld(cs_main);
    int nDoS = 0;
    if (state.IsInvalid(nDoS))
...
// bip135 begin
/** Check for conspicuous versionbit signal events in last 100 blocks and alert. */
void BobtailCheckAndAlertUnknownVersionbits(const CChainParams &chainParams, const CBlockIndex *chainTip)
{
    static bool fWarned = false;
    int nUpgraded = 0;
    bool upgradedEval = false;
    ...
    if (pindex->nHeight >= BIT_WARNING_WINDOW)
    {
        for (int bit = 0; bit < Consensus::MAX_VERSION_BITS_DEPLOYMENTS; bit++)
        {
            if (!IsConfiguredDeployment(chainParams.GetConsensus(), bit))
            {
                const CBlockIndex *iindex = pindex; // iterating index, reset to chain tip
                // set count for this bit to 0
                unknownBobtailFork[bit].unknownBobtailForkSignalStrength = 0;
                for (int i = 0; i < BIT_WARNING_WINDOW && iindex != nullptr; i++)
                {
                    unknownBobtailFork[bit].unknownBobtailForkSignalStrength += ((iindex->nVersion >> bit) & 0x1);
                    if (!upgradedEval)
                    {
                        // do the old "unexpected block version" counting only during first bit walk
                        int32_t nExpectedVersion =
                ...
                upgradedEval = true; // only do the unexpected version checks once during bit loop
                if (unknownBobtailFork[bit].unknownBobtailForkSignalFirstDetected && !unknownBobtailFork[bit].unknownBobtailForkSignalLost &&
                    unknownBobtailFork[bit].unknownBobtailForkSignalStrength == 0)
                {
                    // report a lost signal
                    LOGA("%s: signal lost for unknown fork (versionbit %i)\n", __func__, bit);
                    unknownBobtailFork[bit].unknownBobtailForkSignalFirstDetected = true;
                    unknownBobtailFork[bit].unknownBobtailForkSignalLost = true; // set it so that we don't report on it again
                }
                // report newly gained / regained signal
                else if ((!unknownBobtailFork[bit].unknownBobtailForkSignalFirstDetected || unknownBobtailFork[bit].unknownBobtailForkSignalLost) &&
                         unknownBobtailFork[bit].unknownBobtailForkSignalStrength > 0)
                {
                    // report a newly detected signal
                    LOGA("%s: new signal detected for unknown fork (versionbit %i) - strength %d/%d\n", __func__, bit,
                        unknownBobtailFork[bit].unknownBobtailForkSignalStrength, BIT_WARNING_WINDOW);
                    // set it so that we don't report on it again
                    unknownBobtailFork[bit].unknownBobtailForkSignalFirstDetected = true;
                    unknownBobtailFork[bit].unknownBobtailForkSignalLost = false;
                }
                else if (unknownBobtailFork[bit].unknownBobtailForkSignalStrength >= 95 &&
                         !unknownBobtailFork[bit].unknownBobtailForkSignalAt95Percent)
                {
                    LOGA("%s: signal for unknown fork (versionbit %i) >= 95%% - strength %d/%d\n", __func__, bit,
                        unknownBobtailFork[bit].unknownBobtailForkSignalStrength, BIT_WARNING_WINDOW);
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt95Percent = true;
                }
                else if (unknownBobtailFork[bit].unknownBobtailForkSignalStrength >= 90 &&
                         !unknownBobtailFork[bit].unknownBobtailForkSignalAt90Percent)
                {
                    LOGA("%s: signal for unknown fork (versionbit %i) >= 90%% - strength %d/%d\n", __func__, bit,
                        unknownBobtailFork[bit].unknownBobtailForkSignalStrength, BIT_WARNING_WINDOW);
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt90Percent = true;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt95Percent = false;
                }
                else if (unknownBobtailFork[bit].unknownBobtailForkSignalStrength >= 70 &&
                         !unknownBobtailFork[bit].unknownBobtailForkSignalAt70Percent)
                {
                    LOGA("%s: signal for unknown fork (versionbit %i) >= 70%% - strength %d/%d\n", __func__, bit,
                        unknownBobtailFork[bit].unknownBobtailForkSignalStrength, BIT_WARNING_WINDOW);
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt70Percent = true;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt90Percent = false;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt95Percent = false;
                }
                else if (unknownBobtailFork[bit].unknownBobtailForkSignalStrength >= 50 &&
                         !unknownBobtailFork[bit].unknownBobtailForkSignalAt50Percent)
                {
                    LOGA("%s: signal for unknown fork (versionbit %i) >= 50%% - strength %d/%d\n", __func__, bit,
                        unknownBobtailFork[bit].unknownBobtailForkSignalStrength, BIT_WARNING_WINDOW);
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt50Percent = true;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt70Percent = false;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt90Percent = false;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt95Percent = false;
                }
                else if (unknownBobtailFork[bit].unknownBobtailForkSignalStrength >= 25 &&
                         !unknownBobtailFork[bit].unknownBobtailForkSignalAt25Percent)
                {
                    LOGA("%s: signal for unknown fork (versionbit %i) >= 25%% - strength %d/%d\n", __func__, bit,
                        unknownBobtailFork[bit].unknownBobtailForkSignalStrength, BIT_WARNING_WINDOW);
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt25Percent = true;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt50Percent = false;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt70Percent = false;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt90Percent = false;
                    unknownBobtailFork[bit].unknownBobtailForkSignalAt95Percent = false;
                    fWarned = false; // turn off to repeat the warning when > 50% again
...
/** Update chainActive and related internal data structures. */
void BobtailUpdateTip(CBlockIndex *pindexNew)
{
    const CChainParams &chainParams = Params();
    chainActive.SetTip(pindexNew);
    ...
    // New best block
    nBobtailTimeBestReceived.store(GetTime());
    mempool.AddTransactionsUpdated(1);
...
static void BobtailResubmitTransactions(CBlock &block)
{
    // To be very safe let's force everything in the mempool to be re-admitted.  This reduces this rare case
    // quickly to a very common operation mode.  If we do not do this, we must guarantee that all tx coming from
    // the block get injected into the mempool to ensure that any mempool tx that relies on an input from this
...
/** Disconnect chainActive's tip. */
bool BobtailDisconnectTip(CValidationState &state, const Consensus::Params &consensusParams, const bool fRollBack)
{
    AssertLockHeld(cs_main);
    AssertLockHeld(PV->cs_blockvalidationthread);
 ...
 * corresponding to pindexNew, to bypass loading it again from disk.
 */
bool BobtailConnectTip(CValidationState &state,
    const CChainParams &chainparams,
    CBlockIndex *pindexNew,
    const CBlock *pblock,
    bool fParallel)
{
    ...
    // Apply the block atomically to the chain state.
    int64_t nBobtailTime2 = GetStopwatchMicros();
    nBobtailTimeReadFromDisk += nBobtailTime2 - nBobtailTime1;
    int64_t nBobtailTime3;
    LOG(BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nBobtailTime2 - nBobtailTime1) * 0.001, nBobtailTimeReadFromDisk * 0.000001);
    {
        CCoinsViewCache view(pcoinsTip);
        bool rv = ConnectBlock(*pblock, state, pindexNew, view, chainparams, false, fParallel);
        GetMainSignals().BlockChecked(*pblock, state);
        ...
        mapBlockSource.erase(pindexNew->GetBlockHash());
        nBobtailTime3 = GetStopwatchMicros();
        nBobtailTimeConnectTotal += nBobtailTime3 - nBobtailTime2;
        LOG(BENCH, "  - Connect total: %.2fms [%.2fs]\n", (nBobtailTime3 - nBobtailTime2) * 0.001, nBobtailTimeConnectTotal * 0.000001);
    }

    int64_t nBobtailTime4 = GetStopwatchMicros();
    nBobtailTimeFlush += nBobtailTime4 - nBobtailTime3;
    LOG(BENCH, "  - Flush: %.2fms [%.2fs]\n", (nBobtailTime4 - nBobtailTime3) * 0.001, nBobtailTimeFlush * 0.000001);
    // Write the chain state to disk, if necessary, and only during IBD, reindex, or importing.
    if (!IsChainNearlySyncd() || fReindex || fImporting)
        if (!FlushStateToDisk(state, FLUSH_STATE_IF_NEEDED))
            return false;
    int64_t nBobtailTime5 = GetStopwatchMicros();
    nBobtailTimeChainState += nBobtailTime5 - nBobtailTime4;
    LOG(BENCH, "  - Writing chainstate: %.2fms [%.2fs]\n", (nBobtailTime5 - nBobtailTime4) * 0.001, nBobtailTimeChainState * 0.000001);
    ...
    int64_t nBobtailTime6 = GetStopwatchMicros();
    nBobtailTimePostConnect += nBobtailTime6 - nBobtailTime5;
    nBobtailTimeTotal += nBobtailTime6 - nBobtailTime1;
    LOG(BENCH, "  - Connect postprocess: %.2fms [%.2fs]\n", (nBobtailTime6 - nBobtailTime5) * 0.001, nBobtailTimePostConnect * 0.000001);
    LOG(BENCH, "- Connect block: %.2fms [%.2fs]\n", (nBobtailTime6 - nBobtailTime1) * 0.001, nBobtailTimeTotal * 0.000001);
...
static void BobtailCheckForkWarningConditionsOnNewFork(CBlockIndex *pindexNewForkTip)
{
    AssertLockHeld(cs_main);
    // If we are on a fork that is sufficiently large, set a warning flag
    CBlockIndex *pfork = pindexNewForkTip;
    ...
    // the 7-block condition and from this always have the most-likely-to-cause-warning fork
    if (pfork &&
        (!pindexBobtailBestForkTip || (pindexBobtailBestForkTip && pindexNewForkTip->nHeight > pindexBobtailBestForkTip->nHeight)) &&
        pindexNewForkTip->nChainWork - pfork->nChainWork > (GetBlockProof(*pfork) * 7) &&
        chainActive.Height() - pindexNewForkTip->nHeight < 72)
    {
        pindexBobtailBestForkTip = pindexNewForkTip;
        pindexBobtailBestForkBase = pfork;
    }
 ...
 * pblock is either nullptr or a pointer to a CBlock corresponding to pindexMostWork.
 */
bool BobtailActivateBestChainStep(CValidationState &state,
    const CChainParams &chainparams,
    CBlockIndex *pindexMostWork,
    const CBlock *pblock,
    bool fParallel)
 ...
 * that is already loaded (to avoid loading it again from disk).
 */
bool BobtailActivateBestChain(CValidationState &state,
    const CChainParams &chainparams,
    const CBlock *pblock,
    bool fParallel,
    CNode *pfrom)
{
    ...
    do
    {
        ...
        if (fParallel && pblock)
        {
            READLOCK(cs_mapBlockIndex);
            std::set<CBlockIndex *, CBlockIndexWorkComparator>::reverse_iterator it = setBobtailBlockIndexCandidates.rbegin();
            while (it != setBobtailBlockIndexCandidates.rend())
            {
                if ((*it)->nChainWork == pindexMostWork->nChainWork)
                    if ((*it)->nSequenceId < pindexMostWork->nSequenceId)
                        pindexMostWork = *it;
